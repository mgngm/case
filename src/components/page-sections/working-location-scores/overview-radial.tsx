...
const OverviewRadial = ({ score, height, width, colorOverrides }: overviewTypes) => {
  ...
  const textColor = colorOverrides?.text ?? 'white';
    ...
    plotOptions: {
      radialBar: {
        hollow: {
          margin: 20,
          size: '72%',
          background: colorOverrides?.hollow ?? '#31185D',
        },
        track: {
          background: colorOverrides?.track ?? '#4D347A',
        },
        dataLabels: {
          name: {
            show: true,
            fontSize: !width && matches ? '18px' : '16px',
            fontFamily: 'mulish',
            fontWeight: 300,
            color: textColor,
            offsetY: !width && matches ? 50 : 35,
          },
          value: {
            offsetY: !width && matches ? 0 : -10,
            color: textColor,
            fontSize: !width && matches ? '70px' : '40px',
            fontWeight: 700,
            fontFamily: 'mulish',
            show: true,
            //We format this this way as it's a number to a string back to a number, then we want to keep the decimal point.
            formatter: (val: number): string => `${(val / 10).toFixed(1)}`,
          },
        },
      },
    },
